...
typedef union {
  ...
  struct {
    uint32_t T : 7; // bit 0 7-bit counter (MSB to LSB)
    uint32_t WDGA : 1; // bit 7 Activation bit
...
typedef union {
  volatile uint32_t REG;
  struct {
    uint32_t W : 7; // bit 0 7-bit window value
    uint32_t WDGTB : 2; // bit 7 Timer Base
    uint32_t EWI : 1; // bit 9 Early Wakeup Interrupt
...
typedef union {
  volatile uint32_t REG;
  struct {
    uint32_t EWI : 1; // bit 0 Early Wakeup Interrupt
